...
import { SavedProject, QuoteData, CompanyProfile, FixturePricing, User, UserSettings } from './types';
...
const parsePromptForQuantities = (text: string): Record<string, number> => {
    ...
    patterns.forEach(p => {
       ...
       if (match) counts[p.id] = parseInt(match[1], 10);
    });
    return counts;
};
...
const App: React.FC = () => {
  ...
  // 3. BRIDGE: Sync Clerk User -> App State
  useEffect(() => {
  ...
  const requestApiKey = async () => {
    if (!import.meta.env.VITE_GEMINI_API_KEY && (window as any).aistudio) {
        try {
      ...
      setError("API Key missing. Check Vercel Env Vars.");
    }
  };
  ...
  const toggleFixture = (fixtureId: string) => {
    setSelectedFixtures(prev => prev.includes(fixtureId) ? prev.filter(id => id !== fixtureId) : [...prev, fixtureId]);
  };
  ...
  const handleGenerate = async () => {
    ...
    if (selectedFixtures.includes('gutter') && !selectedFixtures.includes('soffit')) {
        activePrompt += `\n\n[HARD CONSTRAINT]: GUTTER LIGHTS ARE ACTIVE, BUT SOFFIT LIGHTS ARE DISABLED. You must ONLY generate lights shining UP from the gutter lip. The underside of the roof eaves (soffits) MUST remain completely dark.`;
    }
    const selectedColor = COLOR_TEMPERATURES.find(c => c.id === colorTemp);
    if (selectedColor && selectedColor.prompt) activePrompt += `\n\nLIGHTING COLOR SETTING: ${selectedColor.prompt}`;
  ...
  const handleGenerateQuote = () => {
    ...
    const defaultQuantities: Record<string, number> = { 'up': 12, 'path': 6, 'gutter': 4, 'soffit': 4, 'hardscape': 8, 'coredrill': 4, 'transformer': 1 };
    const hasParsedCounts = Object.keys(parsedCounts).length > 0;

    const lineItems = pricing.map(def => {
         if (def.fixtureType === 'transformer') return { ...def, quantity: 1 };
         if (hasParsedCounts) {
             const qty = parsedCounts[def.id] || 0;
             return { ...def, quantity: qty };
         } else {
             if (selectedFixtures.includes(def.fixtureType)) return { ...def, quantity: defaultQuantities[def.fixtureType] || 1 };
             return { ...def, quantity: 0 };
         }
    }).filter(item => item.quantity > 0);

    const newQuote: QuoteData = {
        lineItems: lineItems.map(i => ({ id: i.id, name: i.name, description: i.description, quantity: i.quantity, unitPrice: i.unitPrice })),
        taxRate: 0.07,
        discount: 0,
  ...
  const handleDeleteProject = (id: string) => {
      ...
      const updatedAllProjects = allProjects.filter((p: SavedProject) => p.id !== id);
      // 3. Save back
      localStorage.setItem("lumina_projects", JSON.stringify(updatedAllProjects));
                  ...
                  <div className="max-w-4xl mx-auto min-h-full p-4 md:p-8 flex flex-col justify-start md:justify-center relative z-10">
                    ...
                    <div className="flex-1 flex flex-col relative bg-black rounded-[32px] overflow-hidden border border-white/10 shadow-2xl animate-in fade-in zoom-in-95 duration-500 min-h-[500px]">
                        <div className="absolute top-6 left-0 right-0 z-40 flex justify-center gap-3 px-4">
                            <button onClick={handleSaveProjectFromEditor} className="bg-black/40 backdrop-blur-md text-white border border-white/10 px-5 py-3 rounded-xl font-bold uppercase tracking-wider text-[10px] md:text-xs hover:bg-white/10 hover:scale-105 transition-all shadow-lg flex items-center gap-2">
                                <FolderPlus className="w-4 h-4 text-[#F6B45A]" />
                                Save Project
                            </button>
                            <button onClick={handleGenerateQuote} className="bg-[#F6B45A] text-[#111] px-5 py-3 rounded-xl font-bold uppercase tracking-wider text-[10px] md:text-xs hover:bg-[#ffc67a] hover:scale-105 transition-all shadow-[0_0_20px_rgba(246,180,90,0.3)] flex items-center gap-2">
                                <FileText className="w-4 h-4" />
                                Generate Quote
                            </button>
                        </div>
                        <div className="flex-1 relative flex items-center justify-center bg-[#050505] overflow-hidden group">
                            <img src={generatedImage} alt="Generated Result" className="w-full h-full object-contain cursor-zoom-in transition-transform duration-300" onClick={() => setIsFullScreen(true)} />
                            {isLoading && (
                        ...
                        <div className="absolute bottom-8 left-1/2 -translate-x-1/2 z-40 flex flex-col items-center gap-3 w-full px-4">
                            <div className="flex items-center gap-3 bg-black/80 backdrop-blur-xl border border-white/10 p-2 rounded-full shadow-2xl">
                                <button onClick={() => { setIsLiked(!isLiked); if (!isLiked) handleDownload(); }} className={`p-3 rounded-full transition-all duration-200 ${isLiked ? 'bg-[#F6B45A] text-[#111]' : 'hover:bg-white/10 text-white'}`} title="Like & Download">
                                    <ThumbsUp className={`w-5 h-5 ${isLiked ? 'fill-current' : ''}`} />
                                </button>
                                <button onClick={() => setShowFeedback(true)} className={`p-3 rounded-full transition-all duration-200 ${showFeedback ? 'bg-red-500/90 text-white' : 'hover:bg-white/10 text-white'}`} title="Changes Needed">
                                    <ThumbsDown className="w-5 h-5" />
                                </button>
                                <div className="w-px h-5 bg-white/10 mx-1"></div>
                                <button onClick={handleCloseResult} className="p-3 rounded-full hover:bg-white/10 text-white transition-all" title="Start Over">
                                    <X className="w-5 h-5" />
                                </button>
                            </div>
                             <button onClick={() => setIsFullScreen(true)} className="flex items-center gap-2 bg-black/40 hover:bg-black/60 backdrop-blur-md text-white/50 hover:text-white px-4 py-2 rounded-full text-[10px] font-bold uppercase tracking-widest transition-all">
                        ...
                        {showFeedback && (
                            <div className="absolute inset-0 z-50 bg-black/90 backdrop-blur-sm flex items-center justify-center p-6 animate-in fade-in duration-300">
                                <div className="w-full max-w-md bg-[#111] rounded-2xl p-6 shadow-2xl border border-white/10">
                                    ...
                                    <textarea value={feedbackText} onChange={(e) => setFeedbackText(e.target.value)} className="w-full h-32 p-4 rounded-xl border border-white/10 bg-black/50 text-white text-sm focus:border-[#F6B45A] focus:ring-1 focus:ring-[#F6B45A] outline-none resize-none mb-4 font-mono" placeholder="Describe specific changes (e.g., 'Remove the tree light', 'Make it warmer')..." autoFocus />
                                    <button onClick={handleFeedbackRegenerate} disabled={!feedbackText.trim() || isLoading} className="w-full bg-[#F6B45A] text-[#111] py-3 rounded-xl font-bold uppercase tracking-wider text-sm hover:bg-[#ffc67a] flex items-center justify-center gap-2 disabled:opacity-50 transition-colors">
                    ...
                    <div className="flex flex-col gap-8 animate-in slide-in-from-bottom-4 fade-in duration-500 pb-20 md:pb-0">
                        <div className="relative">
                            <ImageUpload currentImage={file} previewUrl={previewUrl} onImageSelect={handleImageSelect} onClear={handleClear} />
                        </div>
                        <div className="flex flex-col gap-6">
                            <div className="flex flex-col gap-3">
                                ...
                                <div className="grid grid-cols-2 md:grid-cols-3 gap-3 p-1">
                                    {FIXTURE_TYPES.map((ft) => {
                                        ...
                                        return (
                                            <button key={ft.id} onClick={() => toggleFixture(ft.id)} className={`py-3 px-4 rounded-xl text-[10px] md:text-xs font-bold uppercase tracking-wider transition-all duration-200 border flex items-center justify-center text-center ${isSelected ? 'bg-[#F6B45A] text-[#111] border-[#F6B45A] shadow-[0_0_15px_rgba(246,180,90,0.3)] scale-[1.02]' : 'bg-[#111] text-gray-300 border-white/10 hover:bg-[#1a1a1a] hover:border-white/20 hover:text-white'}`}>
                                                {ft.label}
                                            </button>
                                        );
                                    })}
                            ...
                            <div className="relative group mt-2">
                                <textarea className="w-full h-16 bg-[#0F0F0F] border border-white/10 rounded-xl p-4 text-sm text-gray-200 placeholder-gray-400 focus:outline-none focus:border-[#F6B45A]/50 focus:ring-1 focus:ring-[#F6B45A]/50 transition-all resize-none font-mono" placeholder="Type of Fixtures and Number of fixtures (e.g. '10 Up lights, 3 Gutter up lights, 6 Path lights')" value={prompt} onChange={(e) => setPrompt(e.target.value)} />
                                <div className="absolute right-3 bottom-3 text-[10px] text-gray-400 font-mono uppercase tracking-widest">Custom Notes</div>
                            </div>
                            {error && (
                 ...
                 <div className="max-w-7xl mx-auto p-4 md:p-10 relative z-10">
                     <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-6 mb-12 border-b border-white/5 pb-8">
                         ...
                         <div className="w-full md:w-96 relative group">
                            ...
                            <input type="text" value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="block w-full pl-10 pr-3 py-3 border border-white/10 rounded-xl leading-5 bg-[#111] text-gray-200 placeholder-gray-400 focus:outline-none focus:bg-black focus:border-[#F6B45A]/50 focus:ring-1 focus:ring-[#F6B45A]/50 sm:text-sm font-mono transition-all" placeholder="Search by ID or Client..." />
                         </div>
                     </div>
                            ...
                            {filteredProjects.map((p, index) => (
                                <div key={p.id} className="group relative bg-[#111]/80 backdrop-blur-sm border border-white/5 rounded-2xl overflow-hidden hover:border-[#F6B45A]/50 transition-all duration-500 hover:shadow-[0_0_30px_rgba(246,180,90,0.1)] flex flex-col">
                                    <div className={`relative aspect-[4/3] w-full overflow-hidden cursor-pointer bg-black`}>
                                        {p.image ? (
                                            <>
                                                <img src={p.image} onClick={() => { setGeneratedImage(p.image); setActiveTab('editor'); }} className="w-full h-full object-cover opacity-80 group-hover:opacity-100 group-hover:scale-105 transition-all duration-700 ease-out" alt="Scene" />
                                                ...
                                                <div className="absolute inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity duration-300 pointer-events-none">
                                                    <div className="bg-[#F6B45A] text-black px-4 py-2 rounded-full font-bold text-[10px] uppercase tracking-widest shadow-lg transform translate-y-4 group-hover:translate-y-0 transition-transform">Load Scene</div>
                                                </div>
                                            </>
                                        ) : (
                                    ...
                                    <div className="p-5 flex flex-col flex-1 border-t border-white/5 bg-[#0a0a0a]">
                                        <div className="flex justify-between items-start mb-4">
                                            ...
                                            <div className="flex items-center gap-1">
                                                <button onClick={() => handleDeleteProject(p.id)} className="p-2 text-gray-400 hover:text-red-500 hover:bg-white/5 rounded-full transition-colors" title="Delete">
                                        ...
                                        <div className="grid grid-cols-2 gap-3 mt-auto">
                                            ...
                                            <div onClick={() => { if (p.quote) { setCurrentQuote(p.quote); if (p.image) setGeneratedImage(p.image); setActiveTab('quotes'); } }} className={`bg-[#151515] p-2 rounded-lg border border-white/5 relative group/quote transition-colors ${p.quote ? 'cursor-pointer hover:border-[#F6B45A]/30 hover:bg-[#F6B45A]/5' : ''}`}>
                                        ...
                                        <div className="flex items-center gap-2 mt-4 pt-4 border-t border-white/5">
                                            <button onClick={() => handleDownloadImage(p)} className="flex-1 bg-white/5 hover:bg-white/10 text-gray-300 hover:text-white py-2 rounded-lg text-[10px] uppercase font-bold tracking-wider flex items-center justify-center gap-2 transition-all group/btn" title="Download Image" disabled={!p.image}>
                                                <ImageIcon className="w-3 h-3 group-hover/btn:text-[#F6B45A]" />
                                                Save Img
                                            </button>
                                            {p.quote && (
                                                <button onClick={() => setPdfProject(p)} className="flex-1 bg-white/5 hover:bg-white/10 text-gray-300 hover:text-white py-2 rounded-lg text-[10px] uppercase font-bold tracking-wider flex items-center justify-center gap-2 transition-all group/btn" title="Download Quote PDF">
