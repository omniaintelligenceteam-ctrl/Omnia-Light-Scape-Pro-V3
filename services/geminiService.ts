...
// ==========================================
// GUTTER LINE DETECTION (Snap-to-Gutter)
// ==========================================
...
export const detectGutterLines = async (
  imageBase64: string,
  imageMimeType: string = 'image/jpeg'
): Promise<GutterDetectionResult> => {
  const ai = new GoogleGenAI({ apiKey: import.meta.env.VITE_GEMINI_API_KEY });

  const gutterPrompt = `Analyze this property photo and detect horizontal gutter lines.

Gutters are U-shaped metal channels running horizontally along rooflines. They are:
- Below the roof edge (lower than shingles)
- Above windows/soffits
- Often darker in color with shadow below
...
Return ONLY valid JSON:
{
  "gutterLines": [
    {
      "id": "gutter_1",
      "y": <Y as percentage 0-100 from top>,
      "startX": <start X percentage>,
      "endX": <end X percentage>,
      "confidence": <0-1>,
...
First-story gutters: typically Y=70-80%. Second-story: Y=35-50%. If none visible, return empty array.`;
  ...
  try {
    ...
    if (response.candidates?.[0]?.content?.parts) {
      ...
      if (textPart?.text) {
        ...
        const result: GutterDetectionResult = JSON.parse(jsonText);
        result.gutterLines = result.gutterLines.map(line => ({
          ...line,
          y: Math.max(0, Math.min(100, line.y)),
          startX: Math.max(0, Math.min(100, line.startX)),
          endX: Math.max(0, Math.min(100, line.endX)),
          confidence: Math.max(0, Math.min(1, line.confidence))
        }));
        return result;
      }
    }
    return { gutterLines: [], hasGutters: false, confidence: 0, warnings: ['No response'] };
  } catch (error) {
    console.error('Gutter detection failed:', error);
    return { gutterLines: [], hasGutters: false, confidence: 0, warnings: ['Detection failed'] };
  }
};
